...
uint8_t setupMPU(MPU9250_t *MPU9250, uint8_t addr) {
	...
	HAL_UART_Transmit(&huart1, (uint8_t *)badmpu1, strlen((char *)badmpu1), 0xFFFF);
        if ((addr>>1 < MPU9250_DEFAULT_ADDRESS) || (addr>>1 > MPU9250_DEFAULT_ADDRESS + 7)) {
        	char badmpu[] = "Bad Address\n\r";
        	HAL_UART_Transmit(&huart1, (uint8_t *)badmpu, strlen((char *)badmpu), 0xFFFF);
            return 0;
        }
        //mpu_i2c_addr = addr;
...
void calibrate(MPU9250_t *MPU9250){
	...
	HAL_UART_Transmit(&huart1, (uint8_t *)agcal, strlen((char *)agcal), 0xFFFF);
	HAL_Delay(5000);
	...
	HAL_UART_Transmit(&huart1, (uint8_t *)agcal1, strlen((char *)agcal1), 0xFFFF);
	HAL_Delay(5000);
...
void print_calibration(MPU9250_t *MPU9250){
	...
	HAL_UART_Transmit(&huart1, (uint8_t *)str, strlen((char *)str), 0xFFFF);
	char str1[] = "\n\r accel bias [g]: ";
	HAL_UART_Transmit(&huart1, (uint8_t *)str1, strlen((char *)str1), 0xFFFF);
	print_float(str, abx);
	print_float(str, aby);
	print_float(str, abz);
	...
	char str2[] = "\n\r gyro bias [deg/s]: ";
	HAL_UART_Transmit(&huart1, (uint8_t *)str2, strlen((char *)str2), 0xFFFF);
	print_float(str, gbx);
	print_float(str, gby);
	print_float(str, gbz);

	char str3[] = "\n\r mag bias [mG]: ";
	HAL_UART_Transmit(&huart1, (uint8_t *)str3, strlen((char *)str3), 0xFFFF);
	print_float(str, mbx);
	print_float(str, mby);
	print_float(str, mbz);

	char str4[] = ("\n\r mag scale []: ");
	HAL_UART_Transmit(&huart1, (uint8_t *)str4, strlen((char *)str4), 0xFFFF);
	print_float(str, msx);
	print_float(str, msy);
	print_float(str, msz);
}
...
void collect_mag_data_to(MPU9250_t *MPU9250){
		...
		HAL_UART_Transmit(&huart1, (uint8_t *)mag_cal, strlen((char *)mag_cal), 0xFFFF);;
	        HAL_Delay(4000);
...
char readByte(uint8_t I2C_ADDRESS, uint8_t RegAddr){
	...
	HAL_I2C_Master_Transmit(&hi2c2, I2C_ADDRESS, (uint8_t *)data_write, 1, 100);
	HAL_I2C_Master_Receive(&hi2c2, (uint16_t)I2C_ADDRESS, (uint8_t *)data, (uint16_t)1, (uint16_t)100);
...
void readBytes(uint8_t I2C_ADDRESS, uint8_t RegAddr, uint8_t count, uint8_t * dest)
{
	...
	HAL_I2C_Master_Transmit(&hi2c2, I2C_ADDRESS, (uint8_t *)data_write, 1, 100);
	HAL_I2C_Master_Receive(&hi2c2, (uint16_t)(I2C_ADDRESS), (uint8_t *)data, count, (uint16_t)100);
...
void writeByte(uint8_t I2C_ADDRESS, uint8_t RegAddr, uint8_t data){
	...
	HAL_I2C_Master_Transmit(&hi2c2, I2C_ADDRESS, (uint8_t *)data_write, 2, 100);
}
